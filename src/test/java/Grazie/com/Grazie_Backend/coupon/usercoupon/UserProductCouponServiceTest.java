...
public class UserProductCouponServiceTest {
    ...
    @Test
    @DisplayName("유저에게 상품쿠폰이 정상적으로 할당 되는지 테스트")
    void testIssueProductCoupon_Success() {
        // Given
        Long userId = 1L;
        Long couponId = 1L;
        String couponType = "PRODUCT";

        User user = new User();
        user.setId(1L);

        ProductCoupon productCoupon = new ProductCoupon();
        productCoupon.setId(couponId);
        productCoupon.setExpirationDate(LocalDate.now().plusDays(10));

        when(userRepository.findById(userId)).thenReturn(Optional.of(user));
        when(productCouponRepository.findById(couponId)).thenReturn(Optional.of(productCoupon));
        when(userCouponRepository.existsByUserAndCoupon(user, productCoupon)).thenReturn(false); // 아직 발급 안된 상태
    ...
    @Test
    @DisplayName("이미 상품 쿠폰이 발급된 유저에게 다시 발급하려고 할 때 예외 발생")
    void testIssueProductCoupon_AlreadyIssued() {
        // Given
        Long userId = 1L;
        Long couponId = 1L;
        String couponType = "PRODUCT";
        ...
        when(userRepository.findById(userId)).thenReturn(Optional.of(user));
        when(productCouponRepository.findById(couponId)).thenReturn(Optional.of(productCoupon));
        when(userCouponRepository.existsByUserAndCoupon(user, productCoupon)).thenReturn(true); // 이미 발급된 상태
